...
@section('content')
    @component('common-components.breadcrumb')
        ...
        @endslot
        @slot('title')
            <a href="{{url()->previous()}}"> <i class="uil uil-arrow-left "></i>
            </a> Profile {{$enroll->student->name_initials}}
        @endslot
    @endcomponent
                    ...
                    <div class="text-center">
                        @if(auth()->user()->hasRole('Admin'))
                            <div class="dropdown float-right">
                                <a class="text-body text-primary dropdown-toggle " href="#" role="button"
                                   data-toggle="dropdown" aria-haspopup="true">
                                    Action <i class="mdi mdi-menu "></i>
                                </a>
                                <div class="dropdown-menu dropdown-menu-right">
                                    @if($enroll->status=='Documents Pending'||$enroll->status=='Processing')
                                        <a class="dropdown-item sa-accept" data-enrollid="{{$enroll->id}}"
                                           data-enrollstatus="ap" href="#"><i
                                                class="mdi mdi-account-check text-success font-size-20"></i> Accept
                                            Application</a>
                                        <a class="dropdown-item sa-accept" data-enrollid="{{$enroll->id}}"
                                           data-enrollstatus="dp" href="#"><i
                                                class="mdi mdi-account-cancel text-warning font-size-20"></i>
                                            Re-submission Request</a>
                                    @endif
                                    <a class="dropdown-item"
                                       href="{{route('admin.students.edit',['sid'=>$enroll->student_id])}}"><i
                                            class="mdi mdi-account-edit font-size-20"></i> Edit Personal Data</a>
                                    @if($enroll->status=='Registered')
                                        <div class="dropdown-divider"></div>
                                        <h5 class="dropdown-header text-warning">Edit Course</h5>
                                        <a class="dropdown-item"
                                           href="{{route('admin.enroll.change.reg',['id'=>$enroll->id])}}"><i
                                                class="mdi mdi-circle-edit-outline text-warning font-size-20"></i> Edit
                                            Registration Number </a>
                                        <a class="dropdown-item"
                                           href="{{route('admin.enroll.change.course',['id'=>$enroll->id])}}"><i
                                                class="mdi mdi-account-settings text-warning font-size-20"></i>Change
                                            Course of Study </a>
                                    @endif
                                    <div class="dropdown-divider"></div>
                                    <h5 class="dropdown-header">Download</h5>
                                    <a class="dropdown-item"
                                       href="{{route('student.registration.download.LetterOfEnrolment',['eid'=>$enroll->id])}}"><i
                                            class="mdi mdi-file-pdf text-dark"></i> Letter of Enrolment</a>
                                    <a class="dropdown-item"
                                       href="{{route('student.registration.download.PersonalData',['eid'=>$enroll->id])}}"><i
                                            class="mdi mdi-file-pdf text-dark"></i> Download Personal Data</a>
                                    <a class="dropdown-item"
                                       href="{{route('student.registration.download.IdentityCardData',['eid'=>$enroll->id])}}"><i
                                            class="mdi mdi-file-pdf text-dark"></i> Download Identity Card Data</a>
                                    <a class="dropdown-item"
                                       href="{{route('student.registration.download.DegreeDeclarationData',['eid'=>$enroll->id])}}"><i
                                            class="mdi mdi-file-pdf text-dark"></i> Download Degree Declaration Data</a>
                                    <a class="dropdown-item"
                                       href="{{route('student.registration.download.NonSubmissionDocumentsData',['eid'=>$enroll->id])}}"><i
                                            class="mdi mdi-file-pdf text-dark"></i> Download Non-Submission Documents
                                        Data</a>

                                    <div class="dropdown-divider"></div>
                                    <h5 class="dropdown-header">Enroll</h5>
                                    <a class="dropdown-item text-danger"
                                       href="{{route('admin.enroll.dropout',['enroll_id'=>$enroll->id])}}"><i
                                            class="mdi mdi-shield-lock"></i> Edit Enroll Status</a>
                        ...
                        <div>
                            @if($enroll->student->student_docs()->where('type','photo')->first() && Storage::disk('docs')->exists($enroll->student->student_docs()->where('type','photo')->first()->name))
                                <a class="image-popup-no-margins"
                                   href="/registration/student/image/{{$enroll->student->student_docs()->where('type','photo')->first()->name}}">
                                    <img
                                        src="/registration/student/image/{{$enroll->student->student_docs()->where('type','photo')->first()->name}}"
                                        alt="{{$enroll->student->student_docs()->where('type','photo')->first()->type}}"
                                        class="img-fluid  img-thumbnail">
                                </a>
                            @else
                                <img class="img-fluid avatar-lg rounded-circle img-thumbnail" alt=""
                                     src="{{ URL::asset('assets/images/users/user.png')}}" width="75">
                            @endif
                ...
                <div class="card-footer">
                    <div class="font-size-10 text-center text-uppercase">
                        <i class="mdi mdi-clock-outline"></i> <span
                            class="text-muted">Updated at {{$enroll->updated_at->diffForHumans()}} </span>
                    </div>
                </div>
            </div>
        </div>
                        ...
                        <div>
                            ...
                            <div class="card-body">
                                @foreach($enroll->student->addresses as $addr)
                                    <div
                                        class="card-title text-secondary mt-4 border-bottom">{{$addr->address_type}}</div>
                                    <div class="row">
                                        <div class="col-md-4">
                                            <div class="mt-4">
                                        ...
                                        @foreach($enroll->student->student_docs()->get() as $doc)
                                            @if(Storage::disk('docs')->exists($doc->name))
                                                <div class="col-md-6">
                                                    ...
                                                    <a href="/registration/student/image/{{$doc->name}}"
                                                       title="{{$doc->name}}"><img
                                                            src="/registration/student/image/{{$doc->name}}"
                                                            alt="{{$doc->type}}" class="rounded-sm img-thumbnail"></a>
                                                </div>
                                            @endif
                                        @endforeach
                                        ...
                                        @foreach(json_decode($enroll->student->comments) as $comment)
                                            <div class="media border-bottom py-4">
                                                <div class="media-body">
                                                    <h5 class="font-size-12 text-uppercase mt-0 mb-1"><b
                                                            class="text-primary">{{$comment->now}}</b>
                                                        on {{$comment->date}} by {{$comment->created_by}} <small
                                                            class="text-muted float-right">{{\Carbon\Carbon::parse($comment->created_at)->diffForHumans()}}</small>
                                                    </h5>
                                                    <p class="text-muted">{{$comment->comments}}</p>
                                                    <div class="text-muted">
                                                        <h6>Previews Data</h6>
                                                        <span
                                                            class="badge badge-soft-dark p-2 m-1">Programme | {{$comment->p_programme}}</span>
                                                        <span
                                                            class="badge badge-soft-dark p-2 m-1">Academic Year| {{$comment->p_ay}}</span>
                                                        <span class="badge badge-soft-dark p-2 m-1">Registration Number | {{$comment->p_reg}}</span>
                                                        <span
                                                            class="badge badge-soft-dark p-2 m-1">Enroll Status | {{$comment->previews}}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        @endforeach
