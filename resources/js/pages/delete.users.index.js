...
$(document).on('click', '.sa-warning', function(event){
    ...
    var exam_row = event.target.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement;
    swal.queue([{
        title: "Are you sure?",
                ...
                Swal.fire({
                    ...
                    preConfirm: function (text) {
                        return new Promise(function (resolve, reject) {
                            if(text == captcha){
                                $.get('/admin/users/delete/'+exam_id)
                                    .done(function (data) {
                                        if(data.code == 200){
                                            $("#exam"+event.target.parentNode.dataset.exam).closest('tr').fadeOut();
                                            Swal.fire("#"+exam_id+" Deleted!", "Your "+data.msg+" data has been deleted.", "success");
                                            resolve()
                                        }else{
